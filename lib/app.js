...
app.get('/owner/:id', async (req, res) => {
    try {
        res.status(200).json(await Owner.findById(req.params.id));
    } catch (e) {
        res.status(500).text(e.message);
    }
})
...
app.get('/dog/:id', async (req, res) => {
    try {
        res.status(200).json(await Dog.findById(req.params.id))
    } catch (e) {
        res.status(500).text(e.message);
    }
})
...
app.put('/dog/:id', async (req, res) => {
    try {
        res.status(200).json(await Dog.update(req.params.id, req.body))
    } catch (e) {
        res.status(500).text(e.message)
    }
})
...
app.delete('/dog/:id', async (req, res) => {
    try {
        res.status(200).json(await Dog.delete(req.params.id));
    } catch (e) {
        res.status(500).text(e.message);
    }
})
